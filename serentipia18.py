...
import subprocess
from models.classifiers12 import get_classifier_and_grid, get_available_classifiers
import joblib
...
def lgbm_optuna_callback(study: optuna.study.Study, trial: optuna.trial.FrozenTrial) -> None:
    """
    Callback para Optuna que poda trials de LightGBM si `num_leaves` es inválido.
    La restricción es: num_leaves <= 2^(max_depth).
    """
    # Solo se aplica a los trials de LightGBM (gb)
    # Asumimos que el nombre del clasificador está implícito en el contexto donde se llama.
    max_depth = trial.params.get("model__max_depth")
    num_leaves = trial.params.get("model__num_leaves")

    if max_depth is not None and num_leaves is not None:
        max_valid_leaves = (2 ** max_depth)
        if num_leaves > max_valid_leaves:
            # Poda el trial si `num_leaves` excede el límite teórico para esa profundidad
            message = (
                f"Trial podado: num_leaves ({num_leaves}) > 2^max_depth ({max_valid_leaves}) "
                f"para max_depth={max_depth}."
            )
...
def train_and_evaluate_pipeline(global_tensor_all_channels: np.ndarray, 
    ...
    for fold_idx, (train_dev_clf_idx_in_cn_ad_df, test_clf_idx_in_cn_ad_df) in enumerate(outer_cv_clf.split(X_classifier_subject_indices_in_cn_ad_df, stratify_key_for_clf_cv)):
        ...
        for current_classifier_type in args.classifier_types:
            ...
            #main_scorer_name = "custom_auc_bal_scorer"
            ...
            optuna_search = OptunaSearchCV(
                ...
                # CORRECT: scoring is a callable and refit is a boolean
            ...
            if current_classifier_type == "gb":
                # Si el clasificador es LightGBM, añadimos nuestro callback específico.
                optuna_callbacks.append(lgbm_optuna_callback)
                logger.info("     Guardarraíl para LightGBM (num_leaves <= 2^max_depth) activado.")
                # --- FIN: Bloque para imprimir hiperparámetros ---
                
                fold_results_clf[args.gridsearch_scoring] = _auc_balacc_scorer(
                    final_clf_model, X_test_for_pipeline, y_test_final
                )
                ...
                # Después de entrenar optuna_search y tener `final_clf_model`
    ...
    if all_folds_metrics:
        ...
        summary_by_clf = metrics_df.groupby('classifier')[args.gridsearch_scoring].mean()
        best_clf_type = summary_by_clf.idxmax()
        logger.info(f"Mejor clasificador según el promedio de '{args.gridsearch_scoring}': {best_clf_type} ({summary_by_clf.max():.4f})")
