...
import subprocess
from models.classifiers11 import get_classifier_and_grid, get_available_classifiers
import joblib
...
def train_and_evaluate_pipeline(global_tensor_all_channels: np.ndarray, 
    ...
    for fold_idx, (train_dev_clf_idx_in_cn_ad_df, test_clf_idx_in_cn_ad_df) in enumerate(outer_cv_clf.split(X_classifier_subject_indices_in_cn_ad_df, stratify_key_for_clf_cv)):
        ...
        try:
            ...
            logger.info(f"  {fold_idx_str} VAE model compiled with torch.compile() for speedup.")
        except Exception:
            logger.info(f"  {fold_idx_str} torch.compile() not available. Running standard VAE model.")
        ...
        for current_classifier_type in args.classifier_types:
            ...
            if not X_test_for_pipeline.empty:
                ...
                y_dev_proba = final_clf_model.predict_proba(X_train_for_pipeline)[:, 1]
                fpr, tpr, thresholds = roc_curve(y_clf_train_dev_all, y_dev_proba)
            ...
            # Tomar una muestra del set de entrenamiento que usaste para el pipeline
            # X_train_for_pipeline es el DataFrame crudo de train/dev
            # ¡Importante! Usar datos *antes* de SMOTE para representar la distribución real.
            # Suponiendo que tienes acceso a los índices de entrenamiento antes de SMOTE.
            # Si no, una muestra de X_train_for_pipeline es una buena aproximación.
            ...
            #joblib.dump(background_df, fold_output_dir / "shap_background_data.joblib")
